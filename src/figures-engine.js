/* Animated stick-figure engine — CC0 figures rigged with keyframe poses.
   Renders <stick-figure ex="goblet-squat" stroke="#2c2c2a" accent="#0f6e56"
   ground="#cfcdc4"></stick-figure>. Loops the critical portion of the lift.
   viewBox 0 0 120 120, matching the source SVG proportions. */
(function () {
  "use strict";
  if (window.__stickFigureEngine) return;
  window.__stickFigureEngine = true;
  ...
  var Fig = function () { return Reflect.construct(HTMLElement, [], Fig); };
  Fig.prototype = Object.create(HTMLElement.prototype);
  Fig.prototype.constructor = Fig;
  Object.setPrototypeOf(Fig, HTMLElement);

  Fig.prototype.connectedCallback = function () {
    ...
    if ("IntersectionObserver" in window) {
      this._io = new IntersectionObserver(function (es) {
        es.forEach(function (e) {
          ...
          if (e.isIntersecting) self._enable(); else self._disable();
        });
      }, { rootMargin: "80px" });
      this._io.observe(this);
    } else { this._enable(); }
  };
  Fig.prototype.disconnectedCallback = function () { this._disable(); if (this._io) this._io.disconnect(); };

  Fig.prototype._theme = function () {
    return {
  ...
  Fig.prototype._rig = function () { return RIGS[this.getAttribute("ex")] || RIGS["isolation"]; };

  Fig.prototype._draw = function (now) {
    var rig = this._rig(); if (!rig) return;
    var speed = parseFloat(this.getAttribute("speed") || "1") || 1;
    var phase = reduce ? 0.42 : (((now - this._start) * speed) % rig.period) / rig.period;
    render(this._svg, rig, poseAt(rig, phase), this._theme());
  };
  Fig.prototype._frame = function (now) { if (this._visible) this._draw(now); };
  Fig.prototype._enable = function () {
    if (reduce) return;
    if (active.indexOf(this) === -1) active.push(this);
    ensure();
  };
  Fig.prototype._disable = function () {
    var i = active.indexOf(this); if (i !== -1) active.splice(i, 1);
  };

  if (!customElements.get("stick-figure")) customElements.define("stick-figure", Fig);
})();
